...
template<class n_option>
void voronoicell_base<n_option>::output_face_areasV(std::vector<fpoint> * out, std::vector<int> * atom_id) {
	neighbor.neighborsV(atom_id);
	fpoint area;
	int i,j,k,l,m,n;
	fpoint ux,uy,uz,vx,vy,vz,wx,wy,wz;
...
template<class n_option>
void voronoicell_base<n_option>::draw_rgl(ostream &os,int res_id,fpoint x,fpoint y,fpoint z) {
	int i,j,k;fpoint ux,uy,uz;
	for(i=0;i<p;i++) {
		ux=x+0.5*pts[3*i];uy=y+0.5*pts[3*i+1];uz=z+0.5*pts[3*i+2];
		for(j=0;j<nu[i];j++) {
			k=ed[i][j];
			if(k<i)
os << res_id << "," <<  ux << "," << uy << "," << uz << ","<< ux << "," << uy << "," << uz << "," << x+0.5*pts[3*k] << "," << y+0.5*pts[3*k+1] << "," << z+0.5*pts[3*k+2] << "\n";
		}
	}
}
...
template<class n_option>
void voronoicell_base<n_option>::output_neighborsV(std::vector<int> * atom_id) {
	neighbor.neighborsV(atom_id);
}
...
void neighbor_track::neighborsV(std::vector<int> * atom_id) {
	int **edp=vc->ed,*nup=vc->nu;
	int i,j,k,l,m;
	for(i=0;i<vc->p;i++) for(j=0;j<nup[i];j++) {
		k=edp[i][j];
		if(k>=0) {
			atom_id->push_back(ne[i][j]);
			edp[i][j]=-1-k;
			l=vc->cycle_up(edp[i][nup[i]+j],k);
			do {
				m=edp[k][l];
